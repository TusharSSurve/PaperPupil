...
export default function MobileNav({ isAuth }: { isAuth: boolean }) {
  ...
  return (
    <div className="sm:hidden">
      ...
      {
        isOpen ? (<div className="fixed animate-in slide-in-from-top-5 fade-in-20 inset-0 z-0 w-full">
          <ul className="absolute bg-white border-b border-zinc-200 shadow-xl grid w-full gap-3 px-10 pt-20 pb-8">
            {
              !isAuth ? (<>
                ...
                <li>
                  <Link onClick={() => closeOnCurrent('/dashboard')} className="flex items-center w-full font-semibold text-green-600" href='/dashboard'>Dashboard</Link>
                </li>
                <li className="my-3 h-px w-full bg-gray-300" />
                <li>

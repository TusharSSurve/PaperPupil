  ...
  return (
    <div ref={ref} className={cn('flex items-end', {
      ...
      <div className={cn('relative flex h-6 w-6 aspect-square items-center justify-center', {
        ...
        {message.isUserMessage ? (<Icons.user className="fill-zinc-200 text-zinc-200 h-3/4 w-3/4" />) : (<Icons.logo className="fill-zinc-300 h-3/4 w-3/4" />)}
      </div>
          ...
          ) : (message.text)}
          {
            message.id !== 'loading-message' ? (
              <div className={cn('text-xs select-none mt-2 w-full text-right', {
                ...
                'text-green-500': !message.isUserMessage
              })}>
                {
